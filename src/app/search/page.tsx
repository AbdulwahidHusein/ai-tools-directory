...
function SearchSkeletonCard() {
  return (
    <div className="bg-white rounded-lg overflow-hidden shadow-sm border border-gray-100 flex flex-col animate-pulse">
      <div className="h-40 relative bg-gray-200"></div>
...
export default function SearchPageClient({
  ...
  const updateSearch = useCallback(() => {
    ...
    // We're simulating it for client-side filtering
    setIsLoading(true);
    
    // In a real application, this would be an API call
    setTimeout(() => {
      // This is just a simulation - in a real app, you'd fetch from the server
      setIsLoading(false);
      
      // Normally you'd get these from the server
      // Here we're just using the initial data but in real app this would be replaced
      // with actual fetched data based on the new parameters
      setResults(initialData);
      setTotalResults(initialTotalResults);
    }, 800);
    
  }, [query, category, selectedTags, sortBy, page, router, initialData, initialTotalResults]);
  ...
  useEffect(() => {
    updateSearch();
  }, [category, selectedTags, sortBy, page, updateSearch]);
              ...
              <form onSubmit={handleSearch} className="relative">
                ...
                <div className="text-left text-white/80 text-sm mt-2 ml-1">
                  ...
                  <span className="inline-flex gap-2 ml-2">
                    {!query ? (
                      <>
                        <button type="button" onClick={() => setQuery('image generation')} className="hover:text-white">image generation</button>
                        <span>•</span>
                        <button type="button" onClick={() => setQuery('chatbot')} className="hover:text-white">chatbot</button>
                        <span>•</span>
                        <button type="button" onClick={() => setQuery('code')} className="hover:text-white">code</button>
                      </>
                    ) : (
